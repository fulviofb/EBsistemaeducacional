...
import { Target, User, Volume2, BookOpen, Printer, ZoomIn, X } from 'lucide-react';
import { lessonGuidance } from '../../data/guidance';
import { usePlanner } from '../../context/PlannerContext';
import type { LessonId } from '../../types';

// Imagens reais do caderno (quando disponíveis)
const CADERNO_IMAGENS: Record<string, string | null> = {
  aula1: '/images/caderno_u1_aula1.png',
  aula2: null,
  aula3: null,
  aula4: null,
};
...
const CADERNO: Record<string, {
  ...
  aula1: {
    ...
    instrucaoAluno: 'COMO TIA BERNADETE, O(A) PROFESSOR(A) CUIDA DE VOCÊ COM CARINHO. FAÇA UM DESENHO DO SEU LUGAR FAVORITO NA ESCOLA E UMA COISA QUE VOCÊ APRENDEU.',
...
export default function TeacherGuidance() {
  ...
  const imagemReal = CADERNO_IMAGENS[lesson];
  ...
  return (
    <div className="animate-fade-in space-y-5">
      ...
      <div className="rounded-2xl overflow-hidden border-2 border-blue-200 shadow-soft">
        <div className="px-4 py-3 flex items-center gap-3 border-b border-blue-200"
          style={{ background: 'linear-gradient(90deg, #1a3a7a, #2a5ab0)' }}>
        ...
        <div className="bg-blue-50 p-4 space-y-4">
          ...
          <div className="bg-blue-100 border border-blue-200 rounded-xl p-3 flex items-start gap-2.5">
            ...
            <div>
              <p className="text-[10px] font-black uppercase tracking-wider text-blue-700 mb-1">
                O que é o Caderno de Atividades
              </p>
              <p className="text-xs text-blue-800 leading-relaxed">
                Material <strong>impresso e ilustrado</strong> entregue a cada aluno.
                Contém 4 atividades — uma por aula — com a citação âncora, instrução ao aluno e espaço
                para realizar a atividade. A equipe CEEE está finalizando o design gráfico final.
              </p>
            </div>
          </div>

          {/* Imagem real OU preview simulado */}
          <div>
            <p className="text-[9px] font-black uppercase tracking-wider text-blue-600 mb-2">
              📖 {imagemReal ? 'Página do Caderno — Aula ' + lessonNum : 'Prévia da ' + caderno.titulo}
            </p>

            {imagemReal ? (
              /* Imagem real do caderno */
              <div className="relative rounded-xl overflow-hidden border-2 border-blue-300 shadow-md group">
                <img
                  src={imagemReal}
                  alt={`Caderno de Atividades — Aula ${lessonNum}`}
                  className="w-full object-contain"
                  onError={(e) => {
                    // Se imagem não carregou ainda, esconde e mostra preview
                ...
                <button
                  ...
                  className="absolute top-2 right-2 bg-black/40 hover:bg-black/60 text-white
                    rounded-lg p-1.5 opacity-0 group-hover:opacity-100 transition-all"
                >
                  <ZoomIn className="w-4 h-4" />
                </button>
                {/* Fallback oculto */}
                <div style={{ display: 'none' }}>
                  <CadernoPreview caderno={caderno} lessonNum={lessonNum} />
                </div>
              </div>
            ) : (
              /* Preview simulado para aulas sem imagem */
              <CadernoPreview caderno={caderno} lessonNum={lessonNum} />
            )}
          </div>
          ...
          <div className="bg-white border border-blue-200 rounded-xl p-3">
            <p className="text-[9px] font-black uppercase tracking-wider text-blue-600 mb-2">
              👩‍🏫 Instrução para o(a) professor(a)
            </p>
      ...
      {zoomAberto && imagemReal && (
        <div
          className="fixed inset-0 bg-black/80 z-[900] flex items-center justify-center p-4"
      ...
      <div className="space-y-4">
        {lessonGuidance.map(lg => (
          <div key={lg.num} className="rounded-2xl overflow-hidden shadow-soft border border-stone-200">
            <div className="px-4 py-3 flex items-center gap-3 bg-white border-b border-stone-100">
              ...
              >
                {String(lg.num).padStart(2, '0')}
              </div>
              <h3 className="text-sm font-black text-stone-800 flex-1">{lg.title}</h3>
            ...
            <div className="bg-white p-4 space-y-4">
              {lg.objectives && lg.objectives.length > 0 && (
                <div>
              ...
              <div>
                ...
                <p className="text-xs text-stone-600 leading-relaxed">{lg.action}</p>
              </div>

              <div className="bg-brand-50 border border-brand-100 rounded-xl p-3">
                <div className="flex items-center gap-1.5 text-[9px] font-black uppercase tracking-wider text-brand-700 mb-1.5">
                  <Volume2 className="w-3 h-3" />
                  Consciência Fonológica — {lg.fono.sound}
                </div>
                <p className="text-xs text-brand-900 leading-relaxed">{lg.fono.text}</p>
              </div>
            </div>
          </div>
        ))}
...
// Componente reutilizável: preview simulado do caderno
function CadernoPreview({ caderno, lessonNum }: {
  caderno: typeof CADERNO['aula1'];
  lessonNum: number;
}) {
  return (
